    ...
    <div className={`flex flex-wrap items-center justify-center gap-2 sm:gap-3 ${className}`}>
      {socialLinks.map((link) => (
        <a
          ...
          className={`inline-flex ${buttonClass} items-center justify-center rounded-xl border border-white/10 bg-zinc-900/50 text-zinc-400 transition-all hover:border-accent/40 hover:text-accent hover:shadow-[0_0_20px_rgba(0,255,136,0.15)]`}
        >
          <SocialIcon name={link.name} className={iconClass} />
        </a>
      ))}
    </div>
  )
...
function SocialIcon({
  ...
  name: (typeof socialLinks)[number]["name"]
  className: string
}) {
  switch (name) {
    case "GitHub":
